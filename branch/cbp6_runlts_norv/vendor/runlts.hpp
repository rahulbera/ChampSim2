...
struct cbp_hist_t {
    ...
    // CHAMPSIM DEVIATION: '= 0' added. cbp_hist_t() is an empty user-provided
    // constructor, so this member is indeterminate under make_shared; it is read
    // at predict() (path_GEHL) before its first write in history_update().
    ...
    // CHAMPSIM DEVIATION: '= {}' added, same reason -- read at predict() via
    // call_stack_hist() before its first write.
...
#include "runlts_interface.hpp" // CHAMPSIM DEVIATION: renamed from my_cond_branch_predictor.h
