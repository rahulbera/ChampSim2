...
// CHAMPSIM DEVIATION: disabled. Upstream's 64KB sibling already ships this
// commented out. Left on, the namespace-scope instance this header declares
// prints a storage banner at static-init time in EVERY simulator binary --
// ChampSim links every module into every binary -- twice over, and before
// ChampSim's own banner. Budget measured once during bring-up: 190.4 KB
// (1559831 bits: TAGE 1354797, LOOP 9984, SC 195050).
//#define PRINTSIZE
...
#define LOGSNB 10        // 1 1K + 2 * 512-entry tables
#define SNB 3
int Sm[SNB] = { 16, 11, 6 };
...
int8_t *TGEHL[TNB];
#define LOGTLOCAL 5
#define NTLOCAL (1<<LOGTLOCAL)  //Number of third local histories
...
#define LOGB 18         // log of number of entries in bimodal predictor
...
#endif
#ifdef PRINTSIZE
    fprintf (stderr, " (TOTAL %d bits %.1f KBs) ", STORAGESIZE,
            (double)STORAGESIZE / 8192.0);
    fprintf (stdout, " (TOTAL %d bits %.1f KBs) ", STORAGESIZE,
            (double)STORAGESIZE / 8192.0);
#endif
