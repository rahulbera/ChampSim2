...
void berti::l1d_init_current_pages_table()
{
  for (int i = 0; i < L1D_CURRENT_PAGES_TABLE_ENTRIES; i++) {
    // DEVIATION from the vendored source: initialize page_addr to an
    // unmatchable sentinel instead of 0. A zeroed table made every entry look
    // like a live mapping of page 0, so the first demand below 4KB matched a
    // dead entry and aborted at berti.cc's NULL ip-table assert. No real
    // page_addr (bounded by 2^52) can equal this sentinel.
    l1d_current_pages_table[i].page_addr = ~(uint64_t)0;
...
{
  // DEVIATION from the vendored source: invalidate page_addr with the same
  // unmatchable sentinel used at init. Upstream left it 0, so a removed entry
  // stayed findable as "page 0" and resurrected with stale ip/first_offset.
  l1d_current_pages_table[index].page_addr = ~(uint64_t)0;
...
void berti::l1d_init_prev_requests_table()
{
  ...
  for (int i = 0; i < L1D_PREV_REQUESTS_TABLE_ENTRIES; i++) {
    ...
    // DEVIATION from the vendored source: zero the payload fields. The
    // backwards scan in l1d_get_berti_prev_requests_table reads .time of every
    // slot, including never-written ones, which was an indeterminate read.
    l1d_prev_requests_table[i].offset = 0;
    l1d_prev_requests_table[i].time = 0;
  }
}
...
void berti::l1d_init_prev_prefetches_table()
{
  ...
  for (int i = 0; i < L1D_PREV_PREFETCHES_TABLE_ENTRIES; i++) {
    ...
    // DEVIATION from the vendored source: zero the payload fields (same
    // indeterminate-read class as the previous-requests table).
    l1d_prev_prefetches_table[i].offset = 0;
    l1d_prev_prefetches_table[i].time_lat = 0;
    l1d_prev_prefetches_table[i].completed = false;
  }
}
...
void berti::l1d_init_record_pages_table()
{
  for (int i = 0; i < L1D_RECORD_PAGES_TABLE_ENTRIES; i++) {
    ...
    // DEVIATION from the vendored source: a lookup for truncated page 0
    // matches on first_offset, which upstream left indeterminate here.
