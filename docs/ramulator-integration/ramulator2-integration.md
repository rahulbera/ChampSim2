...
function creates its nested mapper without adopting it. After the close-out the
last step uses `lsan-rit.supp`, which suppresses only allocations under that
function: locally the step then exits 0, and LeakSanitizer lists 20 allocations
and 2,028 bytes as suppressed. A control program leaking from both that function
and another still failed on the other leak.
...
| Hosted/clean-host checks | Open | Local builds, mode flips including `RAMULATOR2_SANITIZE`, and the `native_sanitize` steps run by hand. | No hosted job has run on any revision; no clean-host reproduction; macOS, Clang and GCC 9/10 not built on these revisions. |
...
1. Run every hosted job (the legacy compiler matrix, `python`, `stats_output`,
   `native` and `native_sanitize`) on the intended merge revision; none has run
   on any revision of this branch.
2. Reproduce the build and the portable checks on a clean supported host from a
   fresh clone and a fresh pinned native checkout, and freeze the exact
   production, native and toolchain identities.
3. Complete the review of the `--toml` output-file handling, which was deferred
   out of this close-out and is not covered by any evidence above.
4. Archive the evidence (section 6), recording each outcome as passed, failed or
